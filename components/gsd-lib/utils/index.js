...
module.exports = {
    ...
    hideName(name) {
        ...
        return name.replace(/(^.{1})(.+)$/g, (...args) => {
            let tempStr = '';
            if (args[2] && args[2].length) {
                tempStr = Array.from({ length: args[2].length }).join('*');
            }
            return args[1] + tempStr+last;
        });
    },
    navigateBack(delta) {
    ...
    getAllUrlParams(url) {
        ...
        for (var i = 0; i < arr.length; i++) {
            ...
            // paramName = paramName.toLowerCase(); // 如果需要转为小写
            // if (typeof paramValue === 'string') paramValue = paramValue.toLowerCase(); // 如果需要转为小写
